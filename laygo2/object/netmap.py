...
#orient : bool
    def __init__(self, via_table:dict, grid_table:dict, orient_first="vertical", layer_names:list=['M1','M2','M3','M4','M5'], net_ignore = []):
        self.layers=dict()
        self.layers_orient=dict()
        self.pins=list()
        self.terminals=list()
        self.vias=list()
        self.port_rects = list()
    ...
    def insert_metal(self, metal):
        _mn = self.grids[metal.layer[0]].mn(metal.xy)
        ...
        if self.layers_orient[metal.layer[0]] == "vertical" and abs(_mn[1][0]-_mn[0][0]) <= abs(_mn[1][1]-_mn[0][1]): # mn_vetical >= mn_horizontal
            return self.layers[metal.layer[0]].insert_metal(self.grids[metal.layer[0]].mn(metal.xy), net_name=metal.netname, metal_ref=metal)
    ...
    def insert_instance_rect(self, inst:dict, Mxy, Mtransform, Mname, libraries:dict, net_extern:dict=None):
        ...
        if libname in libraries:
            ...
            if os.path.exists(filename):
                with open(filename, 'r') as stream:
                    ...
                    cell = db[libname][cellname]
            else:
                print("Error! no such path: "+filename)
                return
                ...
                if sub_blocks.__class__ is dict:     
                    if net_extern.__class__ is dict:    
                        for block in sub_blocks.values():
                            ...
                            self.insert_instance_rect(block, xy, transform, blockName, libraries, net_block)
                    else:
                        for block in sub_blocks.values():
                            if 'pins' in block.keys():
                                _pins_extern = block['pins']
            ...
            if 'ports' in cell:
                if net_extern.__class__ is dict: # sub-block with external pins
                    for pinName in cell['ports'].keys():
                        ...
                        if len(ports) < 1:
                            continue
                        elif net_extern in self.net_ignore:
                            continue
                        _netname_extern = net_extern[ports[0]['netname']]
                        for metal in ports:
                            _layer = metal['layer']
        ...
        for velem in vinst.native_elements.values():
            if velem.__class__ == laygo2.object.Rect: #insert metals of a virtual instance
                if velem.layer[0] in self.layers:
                    _mn = self.translate_rect_vinst(velem, vinst)
    ...
    def import_from_design(cls, dsn, grid_table, via_table, orient_first="vertical", layer_names=['M3','M4','M5'], net_ignore = [],lib_ref:str = None):
        nMap = cls(via_table=via_table, grid_table = grid_table, orient_first=orient_first, layer_names=layer_names, net_ignore = net_ignore)
        ...
        for rect in dsn.rects.values():
            if rect.layer[0] in layer_names:
                ...
                continue
        # TODO: import metals from subblock using YAML file
        ...
        for _instName, _inst in dsn.instances.items():
            if _inst.shape is None:
                ...
                nMap.insert_instance_rect(inst, np.array([0,0]), np.array([[1,0],[0,1]]), inst['name'], libraries)
